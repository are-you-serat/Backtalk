        ...
        Scaffold(
            ...
            Column(modifier = Modifier
                ...
                Row(modifier = Modifier.padding(horizontal = 16.dp)) {
                    FilterChip(
                        selected = state.includeReplies,
                        onClick = { viewModel.onEvent(ThreadsUiEvent.ToggleIncludeReplies(!state.includeReplies)) },
                        label = { Text(stringResource(R.string.threads_filter_replies)) }
                    )
                    Spacer(modifier = Modifier.width(8.dp))
                    FilterChip(
                        selected = state.groupByTime,
                        onClick = { viewModel.onEvent(ThreadsUiEvent.ToggleGroupByTime(!state.groupByTime)) },
                        label = { Text(stringResource(R.string.threads_filter_time)) }
                    )
                }
